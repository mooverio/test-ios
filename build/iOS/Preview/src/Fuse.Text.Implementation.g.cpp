...
{
    return [] (void* font, const uint16_t* text, int offset, int length, bool ltr) -> void*
    {
        hb_buffer_t* buffer = hb_buffer_create();
        			hb_buffer_pre_allocate(buffer, (unsigned int)length);
        
        			hb_buffer_add_utf16(buffer, text + offset, length, 0, -1);
        			hb_buffer_set_direction(buffer, ltr ? HB_DIRECTION_LTR : HB_DIRECTION_RTL);
        			hb_buffer_guess_segment_properties(buffer);
        			hb_shape((hb_font_t*)font, buffer, NULL, 0);
        
        			return (void*)buffer;
    } (font, (uint16_t*)(text == nullptr ? nullptr : text->Ptr()), offset, length, ltr);
}
...
{
    return [] (const uint16_t* text, int offset, int length, uint8_t* outBitArray) -> int
    {
        ::UErrorCode error = U_ZERO_ERROR;
        			...
        			ubrk_close(bi);
        			return U_ZERO_ERROR;
    } ((uint16_t*)(text == nullptr ? nullptr : text->Ptr()), offset, length, (uint8_t*)(outBitArray == nullptr ? nullptr : outBitArray->Ptr()));
}
// }
